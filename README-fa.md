...
حساب هاستینگ خودت می‌سازد، تمام متغیرهای محیطی را تنظیم می‌کند، دامنه می‌گیرد و در انتها
...
### 🔀 ترانسپورت‌ها
- `VLESS + WS + TLS`
- `VLESS + XHTTP + TLS` *(پیش‌فرض stream-up، حالت packet-up هم موجود)*
...
| `XHTTP_MODE` | `stream-up` | اختیاری، `stream-up` یا `packet-up` |
...
| `XHTTP + TLS` | فقط کانفیگ‌های `type=xhttp`، با حالتی که `XHTTP_MODE` تعیین می‌کند |
...
### مصرف رکوئست — اگر رله کلادفلر دارید مهم است

ورکر کلادفلر بر اساس تعداد رکوئست محاسبه می‌شود و تفاوت ترنسپورت‌ها بسیار زیاد است:

| مسیر | تعداد رکوئست به‌ازای هر اتصال |
|---|---|
| `ws` | **۱** رکوئست، برای تمام عمر تونل هر چقدر هم طول بکشد |
| `xhttp` با `mode=stream-up` | **۲** رکوئست — یک `GET` استریمی برای دریافت، یک `POST` بلندمدت برای ارسال |
| `xhttp` با `mode=packet-up` | **۱ + یکی برای هر تکه آپلود** — دقیقه‌ای صدها رکوئست برای هر کاربر فعال |

متغیر `XHTTP_MODE` تعیین می‌کند کانفیگ‌های تولیدشده چه حالتی را اعلام کنند و پیش‌فرض آن
`stream-up` است. سرور **هر دو** شکل ارسال را می‌پذیرد، مهم نیست این متغیر چه باشد؛ بنابراین
کانفیگ‌هایی که قبلاً به کاربران داده‌اید بعد از آپدیت هم کار می‌کنند و کاربران فقط باید یک بار
لینک اشتراک را به‌روز کنند تا روی حالت کم‌مصرف بیایند. حالت فعال در بخش
**تنظیمات پنل ← اطلاعات سرور** نمایش داده می‌شود.

> [!WARNING]
> **XHTTP آزمایشی است.** در `stream-up` تمام مسیر ارسال روی یک رکوئست می‌ماند که بدنه‌اش
> تدریجی خوانده می‌شود؛ در `packet-up` از `POST`های شماره‌دار با بافر مرتب‌سازی استفاده
> می‌شود. در هر دو حالت مسیر دریافت یک `GET` استریمی است، پس اگر پروکسی یا CDN در مسیر
> پاسخ‌ها را بافر کند مسیر دریافت قفل می‌شود. `stream-up` علاوه بر آن نیاز دارد بدنهٔ
> *رکوئست* هم بافر نشود. **WS مسیر قابل‌اعتماد است** — اگر کاربری مشکل داشت، روی WS بگذارش.

---

## 📦 پشتیبان‌گیری و بازگردانی

از **تنظیمات پنل ← پشتیبان‌گیری** یک فایل `.ixpbak` می‌گیرید که همهٔ کاربران را همراه
**UUID و توکن اشتراکشان**، تمام آی‌پی‌های تمیز با برچسب و وضعیت فعال/غیرفعال، هش رمز پنل و
متغیرهای محیطی نگه می‌دارد. دو گزینه هم دارید: ذخیره نکردن رمز، و اضافه کردن تاریخچهٔ مصرف.

از **تنظیمات پنل ← بازگردانی** همان فایل را برمی‌گردانید. با انتخاب فایل فقط *پیش‌نمایش*
گرفته می‌شود — تعداد کاربران، تاریخ ساخت، و هر متغیر محیطی که با هاست فعلی تفاوت دارد — و تا
تأیید نکنید هیچ چیزی در دیتابیس نوشته نمی‌شود. دو حالت دارد:

| حالت | نتیجه |
|---|---|
| ادغام | کاربرانی که با نام یا UUID تطبیق داشته باشند به‌روز و بقیه اضافه می‌شوند |
| جایگزینی | ابتدا همهٔ کاربران و آی‌پی‌های تمیز فعلی حذف می‌شوند |

چون UUID و توکن اشتراک حفظ می‌شود، **کانفیگ‌هایی که روی گوشی کاربران نصب است بعد از مهاجرت
هم کار می‌کند** — فقط اگر `WS_PATH`/`XHTTP_PATH` عوض شده باشد باید لینک را به‌روز کنند، که
همان را پیش‌نمایش به شما نشان می‌دهد.

فایل یک checksum از نوع SHA-256 روی محتوایش دارد؛ فایل نصفه یا دست‌کاری‌شده رد می‌شود مگر با
گزینهٔ `force` بازگردانی کنید. هر بازگردانی داخل یک تراکنش انجام می‌شود، پس اگر خطایی رخ دهد
دیتابیس دست‌نخورده می‌ماند. ردیف‌های خراب هم فقط شمرده و رد می‌شوند و کل عملیات را خراب نمی‌کنند.

متغیرهای محیطی (`DOMAIN`، `WS_PATH`، `XHTTP_PATH`، `XHTTP_MODE`، `RELAY_DOMAIN` و …) فقط
برای اطلاع ذخیره می‌شوند — پنل نمی‌تواند متغیرهای پلتفرم خودش را عوض کند، پس آن‌ها را دستی در
هاست جدید ست کنید.
